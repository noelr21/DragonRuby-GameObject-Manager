    ...
    def tick args
        ...
        if (args.inputs.mouse.button_left)
            if (@dragStarted)
                ...
                width = @w + xDistance
                height = @h + yDistance
                args.outputs.labels << [200, 1000, "width: #{width}", 0, 0, 0, 255]
                args.outputs.labels << [200, 900, "height: #{height}", 0, 0, 0, 255]
                args.outputs.labels << [100, 500, "Hello World #{xDistance}", 0, 0, 0, 255]

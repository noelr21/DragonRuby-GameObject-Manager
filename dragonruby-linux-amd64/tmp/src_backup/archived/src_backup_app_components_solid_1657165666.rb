    ...
    def tick args
        ...
        if (args.inputs.mouse.button_left)
            if (@dragStarted)
                ...
                # If moving left or right change position and if moving right or up change size, then use 
                xDistance = args.inputs.mouse.x - @startCornerPos[:x]
                yDistance = args.inputs.mouse.y - @startCornerPos[:y]
                if (xDistance < 0)
                    @x = args.inputs.mouse.x - @startCornerPos[:x]
                else
                    @w = @w + xDistance
                end
                if (yDistance < 0)
                    @y = args.inputs.mouse.y - @startCornerPos[:y]
                else
                    @h = @h + yDistance
                end
                # args.outputs.labels << [50, 700, "width: #{width}", 0, 0, 0, 255]
                # args.outputs.labels << [50, 600, "height: #{height}", 0, 0, 0, 255]

    ...
    def tick args
        ...
        if (args.inputs.mouse.button_left)
            if (@dragStarted)
                ...
                width = @w + xDistance * moveSpeed
                height = @h + yDistance * moveSpeed
                args.outputs.labels << [10, 1100, "width: #{width}", "string", 0, 0, 255]
                args.outputs.labels << [10, 1000, "height: #{height}", 0, 0, 0, 255]
